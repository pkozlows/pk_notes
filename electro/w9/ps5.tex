...
The potential plot will resemble $-\ln(x)$, so decreasing towards the cathode.\\
\subsection{}
b) Calculate the resistance drop across the device, i.e., $I \Delta \phi$ given your calculated potential function. How would you expect this to change with more concentrated salt? If I apply 1 A of current, how does this value compare to the thermodynamic water splitting potential?
\subsubsection{Solution}    
Here, we need to compute:
\begin{align}
    \Delta \phi &= \phi(L) - \phi(0) \\
&= -\frac{I R T L}{3 D F^2\left(c_L-c_0\right)} \ln \left(\frac{c_L}{c_0}\right) \\
\implies I \Delta \phi &= -\frac{I^2 R T L}{3 D F^2\left(c_L-c_0\right)} \ln \left(\frac{c_L}{c_0}\right)
\end{align}
So with more concentrated salt (larger $c_L$ ), the resistance drop will become smaller because while the logarithmic term will increase, the denominator term $\left(c_L-c_0\right)$ will increase more rapidly. I am not sure how to compare this to the water splitting potential unless I had access to D and $c_0$.
...
a. There are two peaks in (I) and four peaks in (II). Please identify what the reduction or oxidation reactions are for each peak. Note that the scan here starts at positive potentials (right side of plot), goes negative, and then returns in the positive direction.\\
