...
We can describe the final equations to implement. The correlation self energy can be computed on the imaginary frequency axis as:
\begin{equation}
    \Sigma_{n n^{\prime}}^c(\mathbf{k}, i \omega)= -\frac{1}{\pi N_{\mathbf{k}}} \sum_{m \mathbf{q}} \int_0^{\infty} d \omega^{\prime} \frac{i \omega+\epsilon_F-\epsilon_{m \mathbf{k}-\mathbf{q}}}{\left(i \omega+\epsilon_F-\epsilon_{m \mathbf{k}-\mathbf{q}}\right)^2+\omega^{\prime 2}} \sum_{P Q} v_P^{n m}\left[\left[\mathbf{I}-\mathbf{\Pi}\left(\mathbf{q}, i \omega^{\prime}\right)\right]_{P Q}^{-1}-\delta_{P Q}\right] v_Q^{m n^{\prime}}
\end{equation}
...
\end{equation}
The integration in Eq. 27 must be performed on a numerical grid. Following Ref., ${ }^{43}$ we employ a
9

modified Gauss-Legendre grid that transforms a standard Gauss-Legendre grid in the range [ $-1,1$ ] to $[0, \infty]$ :
$$
\tilde{x}_i=\frac{1+x_i}{2\left(1-x_i\right)}, \quad \tilde{w}_i=\frac{w_i}{\left(1-x_i\right)^2} .
$$
$\left\{x_i, w_i\right\}$ are the original Gauss-Legendre abscissas and weights of the grid points in the range $[-1,1]$, and $\left\{\tilde{x}_i, \tilde{w}_i\right\}$ are the transformed abscissas and weights of the grid points used for integrating Eq. 27. In this work, $N_G=100$ grid points were used for all reported results unless specified.

The real-frequency self-energy is then obtained through an analytic continuation scheme by fitting to an analytic function defined on the full complex plane. We fit the self-energy matrix elements to $N$-point Padé approximants using Thiele's reciprocal difference method (see Appendix of Ref. ${ }^{62}$ for the detailed algorithm):
$$
\Sigma_{n r^{\prime}}(\mathbf{k}, \mathbf{z})=\frac{a_0+a_1 \cdot \mathbf{z}+\ldots+a_{(N-1) / 2} \cdot z^{(N-1) / 2}}{1+b_1 \cdot \mathbf{z}+\ldots+b_{N / 2} \cdot z^{N / 2}}
$$
where $z$ is any complex frequency, $\left\{a_i, b_i\right\}$ are Padé coefficients that need to be fitted. In this work, we use $N=18$ Padé approximants, which requires $18 \mathbf{\Sigma}_{m n^{\prime}}(\mathbf{k}, \mathbf{z}=i \omega)$ data points as input to the fit. We choose these 18 imaginary frequency points to be the positions of the modified GaussLegendre grid points. Once the Padé polynomials are fitted, the real-frequency self-energy is easily calculated:
$$
\mathbf{\Sigma}_{n n^{\prime}}(\mathbf{k}, \omega)=\frac{a_0+a_1 \cdot \omega+\ldots+a_{(N-1) / 2} \cdot \omega^{(N-1) / 2}}{1+b_1 \cdot \omega+\ldots+b_{N / 2} \cdot \omega^{N / 2}}
$$
% $\begin{aligned} \mathbf{\Sigma}_{n n^{\prime}}^c(\mathbf{k}, i \omega)= & -\frac{1}{\pi N_{\mathbf{k}}} \sum_{m \mathbf{q}} \int_0^{\infty} d \omega^{\prime} \frac{i \omega+\epsilon_F-\epsilon_{m \mathbf{k}-\mathbf{q}}}{\left(i \omega+\epsilon_F-\epsilon_{m \mathbf{k}-\mathbf{q}}\right)^2+\omega^{\prime 2}} \\ & \times \sum_{P Q} v_P^{n m}\left[\left[\mathbf{I}-\mathbf{\Pi}\left(\mathbf{q}, i \omega^{\prime}\right)\right]_{P Q}^{-1}-\delta_{P Q}\right] v_Q^{m n^{\prime}}\end{aligned}$
...
We can arrive at
\begin{align}
\Sigma_{n n^{\prime}}^c(\mathbf{k}, i \omega)= & -\frac{1}{\pi N_{\mathbf{k}}} \sum_{m \mathbf{q}} \int_0^{\infty} d \omega^{\prime} \frac{i \omega+\epsilon_F-\epsilon_{m \mathbf{k}-\mathbf{q}}}{\left(i \omega+\epsilon_F-\epsilon_{m \mathbf{k}-\mathbf{q}}\right)^2+\omega^{\prime 2}} \\ & \times \sum_{P Q} v_P^{n m}\left[\left[\mathbf{I}-\mathbf{\Pi}\left(\mathbf{q}, i \omega^{\prime}\right)\right]_{P Q}^{-1}-\delta_{P Q}\right] v_Q^{m n^{\prime}}
\end{align}
\subsection{Finite size corrections}
The head correction to the correlation self-energy, which is just diagonal, is given as
\begin{align}
    \bm{\Sigma}_{n n}^{\mathrm{head}}(\mathbf{k}, i \omega)=- & \frac{1}{\pi^2}\left(\frac{6 \pi^2}{\Omega_{\mathrm{cell}} N_{\mathbf{k}}}\right)^{1 / 3} \int_{-\infty}^{\infty} d \omega^{\prime} \frac{\epsilon_{00}^{-1}(\mathbf{q}=\mathbf{0}, i \omega)-1}{i\left(\omega+\omega^{\prime}\right)+\epsilon_F-\epsilon_{n \mathbf{k}}} . 
\end{align}
Then the wings contribution to the correlation self-energy is given as
\begin{align}
\bm{\Sigma}_{n n^{\prime}}^{\text {wings }}(\mathbf{k}, i \omega)= & -\frac{1}{\pi} \sqrt{\frac{\Omega_{\mathrm{cell}}}{4 \pi^3}}\left(\frac{6 \pi^2}{\Omega_{\mathrm{cell}} N_{\mathbf{k}}}\right)^{2 / 3} \sum_P \int_{-\infty}^{\infty} d \omega^{\prime} \frac{\operatorname{Re}\left[v_P^{n n^{\prime}} \epsilon_{P 0}^{-1}(\mathbf{q}=\mathbf{0}, i \omega)\right]}{i\left(\omega+\omega^{\prime}\right)+\epsilon_F-\epsilon_{n^{\prime} \mathbf{k}}} .
\end{align}
For the exchange self-energy, we only apply the following corr ection to the diagonal elements of the occupied orbitals:
\begin{equation}
    \bm{\Sigma}^x_{n n}(\mathbf{k}, i \omega)=-\frac{2}{\pi}\left(\frac{6 \pi^2}{\Omega_{\mathrm{cell}} N_{\mathbf{k}}}\right)^{1 / 3}
\end{equation}


% Here, $P$ refers to the Gaussian auxiliary basis and $v_P^{n n^{\prime}}$ is the GDF integral defined as $v_{P 0}^{n k, n^{\prime} \mathbf{k}}$. The $\mathbf{G}=\mathbf{0}$ Coulomb divergence correction to the exchange self-energy is derived similarly:
% $$
% \begin{equation*}
% \Sigma_{i i}^x(\mathbf{k}, i \omega)=-\frac{2}{\pi}\left(\frac{6 \pi^2}{\Omega_{\mathrm{cell}} N_{\mathbf{k}}}\right)^{1 / 3}, \tag{46}
% \end{equation*}
% $$
% which is a constant that only applies to the diagonal occupied elements of the exchange self-energy.
% The head and wings of the dielectric function in the $\mathbf{q}=\mathbf{0}$ limit are computed as
% $$
% \begin{align*}
% & h_{00}(\mathbf{q}=\mathbf{0}, i \omega)=1-\lim _{\mathbf{q} \rightarrow \mathbf{0}} \frac{4 \pi}{|\mathbf{q}|^2} \chi_{00}(\mathbf{q} \rightarrow \mathbf{0}, i \omega),  \tag{47}\\
% & w_{P 0}(\mathbf{q}=\mathbf{0}, i \omega)=\lim _{\mathbf{q} \rightarrow \mathbf{0}} \frac{\sqrt{4 \pi}}{|\mathbf{q}|} \chi_{P 0}(\mathbf{q} \rightarrow \mathbf{0}, i \omega), \tag{48}
% \end{align*}
% $$
% where the polarizability is
% $$
% \begin{align*}
% \chi_{00}(\mathbf{q} \rightarrow \mathbf{0}, i \omega)= & \frac{2}{N_{\mathbf{k}} \Omega_{\mathrm{cell}}} \sum_{\mathbf{k}} \sum_i^{\mathrm{occ}} \sum_a^{\mathrm{vir}} \rho_{a \mathbf{k}-\mathbf{q}}^{i \mathbf{k}}(\mathbf{0})  \tag{49}\\
% & \times \frac{\epsilon_{i \mathbf{k}}-\epsilon_{a \mathbf{k}}}{\omega^2+\left(\epsilon_{i \mathbf{k}}-\epsilon_{a \mathbf{k}}\right)^2} \rho_{a \mathbf{k}-\mathbf{q}}^{i \mathbf{k} *}(\mathbf{0}), \\
% \chi_{P \mathbf{0}}(\mathbf{q} \rightarrow \mathbf{0}, i \omega)= & \frac{2}{N_{\mathbf{k}} \Omega_{\mathrm{cell}}^{1 / 2}} \sum_{\mathbf{k}} \sum_i^{\mathrm{occ}} \sum_a^{\mathrm{vir}} v_P^{i a}  \tag{50}\\
% & \times \frac{\epsilon_{i \mathbf{k}}-\epsilon_{a \mathbf{k}}}{\omega^2+\left(\epsilon_{i \mathbf{k}}-\epsilon_{a \mathbf{k}}\right)^2} \rho_{a \mathbf{k}-\mathbf{q}}^{i \mathbf{k} *}(\mathbf{0}) .
% \end{align*}
% $$

% In Eqs. 47 and 48 we evaluate the limit using $\mathbf{q}=(0.001,0,0)$; we neglect the anisotropy of this limit for simplicity in this work, although anisotropic corrections can be obtained as discussed in Ref. ${ }^{66}$ The pair density matrix in the long-wavelength limit is computed using $\mathbf{k} \cdot \mathbf{p}$ perturbation theory as described in Ref.: ${ }^{67}$
% $$
% \begin{equation*}
% \left.\rho_{a \mathbf{k}-\mathbf{q}}^{i \mathbf{k}}(\mathbf{0})\right|_{\mathbf{q} \rightarrow \mathbf{0}}=\frac{-i \mathbf{q} \cdot\left(\psi_{i \mathbf{k}}|\nabla| \psi_{a \mathbf{k}}\right)}{\epsilon_{a \mathbf{k}}-\epsilon_{i \mathbf{k}}} \tag{51}
% \end{equation*}
% $$

% Finally, we note that $\epsilon_{\mathbf{0 0}}^{-1}(\mathbf{q}=\mathbf{0}, i \omega)$ and $\epsilon_{\boldsymbol{P 0}}^{-1}(\mathbf{q}=\mathbf{0}, i \omega)$ in Eqs. 44 and 45 are not simple inverses of $h_{\mathbf{0 0}}$ and $w_{\boldsymbol{P 0}}$ in Eqs. 47 and 48 . Instead, they are matrix elements of the inverse of the full dielectric matrix: ${ }^{23,48}$
% $$
% \epsilon_{\mathrm{full}}(\mathbf{q}=\mathbf{0}, i \omega)=\left[\begin{array}{cc}
% h_{00} & w_{P 0}^{\dagger}  \tag{52}\\
% w_{P 0} & B_{P Q}
% \end{array}\right],
% $$
% where $\boldsymbol{B}_{P Q}=[\mathbf{I}-\mathbf{\Pi}]_{P Q}$ is the body of the dielectric function computed in the Gaussian auxiliary basis (see Eq. 24). By inverting Eq. 52, one obtains $\epsilon_{\mathbf{0 0}}^{-1}(\mathbf{q}=\mathbf{0}, i \omega)$ and $\epsilon_{P \mathbf{0}}^{-1}(\mathbf{q}=\mathbf{0}, i \omega)$ as
% $$
% \begin{gather*}
% \epsilon_{\mathbf{0 0}}^{-1}=1 /\left(h_{\mathbf{0 0}}-\sum_{P Q} w_{P \mathbf{0}}^{\dagger} B_{P Q}^{-1} w_{Q \mathbf{0}}\right)  \tag{53}\\
% \epsilon_{P \mathbf{0}}^{-1}=-\epsilon_{\mathbf{0 0}}^{-1} \sum_Q B_{P Q}^{-1} w_{Q \mathbf{0}} \tag{54}
% \end{gather*}
% $$

% The Coulomb divergence corrections in the $G_0 W_0$-CD approach are also implemented in a similar manner.
% $\begin{aligned} \Sigma_{n n^{\prime}}^c(\mathbf{k}, i \omega)= & -\frac{1}{\pi N_{\mathbf{k}}} \sum_{m \mathbf{q}} \int_0^{\infty} d \omega^{\prime} \frac{i \omega+\epsilon_F-\epsilon_{m \mathbf{k}-\mathbf{q}}}{\left(i \omega+\epsilon_F-\epsilon_{m \mathbf{k}-\mathbf{q}}\right)^2+\omega^{\prime 2}} \\ & \times \sum_{P Q} v_P^{n m}\left[\left[\mathbf{I}-\mathbf{\Pi}\left(\mathbf{q}, i \omega^{\prime}\right)\right]_{P Q}^{-1}-\delta_{P Q}\right] v_Q^{m n^{\prime}}\end{aligned}$
