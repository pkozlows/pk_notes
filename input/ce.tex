...
Here we start with $G_0 = G^{SRG-MP2}$. This means the single particle energies are dressed, but the two-electron integrals remain the same as in HF. So, we are solving
\begin{equation}G^{SRG-MP2+C} = G^{SRG-MP2} e^{C^{SRG-MP2}}\end{equation}
where $C^{SRG-MP2}$ is given by eqn.~\ref{eq:cumulant_2nd_2} with the single particle energies evaluated at the SRG-MP2 level and the two-electron integrals evaluated at the HF level.
\subsection{BWs2+C}
