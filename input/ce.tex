...
&\equiv \ket{\Psi_p}
\end{align}
where in \ref{eq:core_hole_approx_1} we approximated the exact vacuum by the Fermi vacuum. \emph{My hypothesis is that by instead approximating the exact ground state by the CC ground state, we would be able to recover the CC Green's function for this problem of the core hole.}
%  and in \ref{eq:core_hole_approx_2} we approximated the bosonic vacuum as unaffected by the removal of the core electron (this would be true for plasmons because the excitation of a core state would only occur at high energies). 
\subsection{Connecting to the CCGF}
Led us perform a double similarity transformation of the Hamiltonian.
...
%  Tolle showed that in finite systems, if one applies the quasi-boson approximation, these can be obtained as RPA quantities. Concretely, if $i,j$ are occupied indices and $a,b$ are virtual indices of $\Phi_0^{N-1}$ and we define
% \begin{align}
...
%     & = \hat{H}_0 + \hat{V},
% \end{align}
% defining $\hat{n}_p = \hat{c}_p^\dagger \hat{c}_p$ as the number operator for the core hole state $p$. The first term of \ref{eq:effective_hamiltonian} describes the single, fermionic core hole, the second term describes the bosonic bath, while the second term is the interaction $\hat{V}$. We can partition into a non-interacting part $\hat{H}_0$ and an interaction $\hat{V}$ as
% \begin{align}
%     \hat{H}_0 &= \epsilon_{p} \hat{n}_p + \sum_{\nu} \Omega_{\nu} \hat{b}_\nu^\dagger \hat{b}_\nu \\
%     \hat{V} &= \sum_{\nu} W_{p\nu} \hat{n}_p \left( \hat{b}_\nu^\dagger + \hat{b}_\nu \right).
% \end{align}
