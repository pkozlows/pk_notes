...
\end{align}
where $\mathbf{k}_b = \mathbf{k}_p  + \mathbf{k}_i - \mathbf{k}_a$ in the first sum and $\mathbf{k}_j = \mathbf{k}_p - \mathbf{k}_i + \mathbf{k}_a$ in the second sum due to momentum conservation. This means we can write down the second order cumulant in the time domain as
\begin{align}
...
We can try to avoid the numerical for year transform by operating directly in the frequency domain. 
We want to get the interacting Green's function in the time domain from the cumulant expansion as
\begin{equation}
...
C_{pp}^{(2)}(t) &\equiv i \int \frac{d\omega}{2\pi} \frac{ \tilde{\Sigma}_{pp}^{c,(2)}\left(\omega+\epsilon_p\right)}{(\omega + i \eta)^2} e^{-i \omega t} \\
 &= \frac{1}{2} \sum_{iab} \left< pi \left| \right| ab \right>^2 \int \frac{d\omega}{2\pi} \frac{i e^{-i \omega t}}{\omega^2\left(\omega-\epsilon_{pi}^{ab}\right)} + \frac{1}{2} \sum_{ija} \left< pa \left| \right| ij \right>^2 \int \frac{d\omega}{2\pi} \frac{i e^{-i \omega t}}{\omega^2\left(\omega-\epsilon_{pa}^{ij}\right)} \\
% \label{eq:cumulant_2nd_1}
&\overset{RHF}{=} \sum_{iab} \frac{\left< pi \left| \right| ab \right>^2}{\left(\epsilon_{pi}^{ab}\right)^2}
\left(e^{-i \epsilon_{pi}^{ab} t} +i \epsilon_{pi}^{ab} t -1 \right)
+\sum_{ija} \frac{\left< pa \left| \right| ij \right>^2}{\left(\epsilon_{pa}^{ij}\right)^2}
\left(e^{-i \epsilon_{pa}^{ij} t} +i \epsilon_{pa}^{ij} t -1 \right) 
\label{eq:cumulant_2nd_final}
\end{align}
where
...
\begin{enumerate}
    ...
    \item Get $G_{pp}(t) = -i \Theta(t) e^{-i \epsilon_p t} e^{C_{pp}^{(2)}(t)}$ with $C_{pp}^{(2)}(t)$ from \ref{eq:cumulant_2nd_final}
    \item $G_{pp}(\omega) = FFT\left[ G_{pp}(t) \right]$
    \item $A_{pp}(\omega) = -\frac{1}{\pi} \operatorname{Im} \left[ G_{pp}(\omega) \right]$
    \item $n_p = \int_{-\infty}^{\infty} f(\omega-\mu) A_{pp}(\omega) d\omega$ with $f(\omega-\mu)$ the Fermi-Dirac distribution at chemical potential $\mu$ (so just a step function at zero temperature)
    \item $P_{\mu \nu} = 2\sum_p C_{\mu p} n_p C_{p \nu}^{T}$
    \item $F_{\mu \nu}=h_{\mu \nu}+\sum_{\rho \sigma}\left(\langle \mu \rho \mid \nu \sigma\rangle-\frac{1}{2}\langle \mu \rho \mid \sigma \nu\rangle\right) P_{\rho \sigma}$
...
\end{align}
where in the first expression, because to start with the index $p$ is already constrained to have spin $+$, so $a$ must be $+$ as well, and we can accept 2 combinations of spin for the two-electron integrals ($\beta , \gamma = +,-$), while in the second expression, we can only accept 1 combinations of $\beta , \gamma = +$.
...
\subsubsection{Sine-cosine basis}
We know that for any plane wave state with $\mathbf{K} \neq 0$, it has the partner $-\mathbf{K}$, and the sine-coosine basis generates the linear combinations
\begin{align}
|\mathbf{K}, s \rangle &= \frac{1}{\sqrt{2}} \left( |\mathbf{K} \rangle + |-\mathbf{K} \rangle \right) \\
|\mathbf{K}, c \rangle &= \frac{1}{\sqrt{2}} \left( -i |\mathbf{K} \rangle + i |-\mathbf{K} \rangle \right)
\end{align}
where $s$ and $c$ denote the sine and cosine functions, respectively. This can be written in matrix form as
\begin{align}
&
\begin{bmatrix}|\mathbf{K},s \rangle \\
|\mathbf{K},c \rangle
\end{bmatrix} = \mathbf{U} \begin{bmatrix} |\mathbf{K} \rangle \\
|-\mathbf{K} \rangle
\end{bmatrix}
\quad \text{with} \quad
...
\subsubsection{HF is the same in both bases}
When I do
...
I reproduce the result from the pyqcpbc fock builder, which uses the Cholesky vectors in the plane wave basis.
\subsubsection{Checking MP2 energy}
the first thing that we can to validate this basis change is to check is whether my MP2 energy is correct. as suggested by Szabo, the expression that we want for a closed shelf system is
\begin{align}
    E_0^{(2)}&=\sum_{i j a b}^{N / 2} \frac{\langle i j \mid a b\rangle \Big( 2\langle a b \mid i j\rangle - \langle a b \mid j i\rangle\Big)}{\epsilon_i + \epsilon_j - \epsilon_a - \epsilon_b} \\
    & = \sum_{i j a b}^{N / 2} \frac{(ia|jb)\Big(2(ia|jb) - (ja|ib)\Big)}{\epsilon_i + \epsilon_j - \epsilon_a - \epsilon_b}
\end{align}
It is invariant to the choice of basis, so when I compute this expression in the sine-cosine basis, I match with the answer given by a reference, such as PySCF. Shu Fay's equation say 
\begin{align}
    \left(p_{\vec{k}} q_{\vec{k}} \mid i_{\vec{k}^{\prime}} j_{\overrightarrow{k^{\prime}}}\right) & =\int_{\Omega} d^2 \vec{r} d^2 r^{\prime} \phi_{p \vec{k}}^*(\vec{r}) \phi_{q k}(\vec{r}) \frac{1}{\left|\vec{r}-r^{\prime}\right|} \phi_{i \vec{k}^{\prime}}^*\left(r^{\prime}\right) \phi_{j \overrightarrow{k^{\prime}}}\left(r^{\prime}\right) \\ & =\frac{1}{\Omega} \int d^2 \vec{r} \phi_{p \vec{k}}^*(\vec{r}) \phi_{q \vec{k}}(\vec{r})\left[\int d^2 r^{\prime} \frac{1}{\left|\vec{r}-r^{\prime}\right|} e^{i\left(\vec{G}_j-\vec{G}_i\right) \cdot r^{\prime}}\right] \\ & =\frac{1}{\Omega} \int_{\Omega} d^2 \vec{r} \phi_{p \vec{k}}^*(\vec{r}) \phi_{q k}(\vec{r}) e^{i\left(\vec{G}_j-\vec{G}_i\right) \cdot \vec{r}} \underbrace{\left[\int_{\Omega} d^2 r^{\prime} \frac{1}{\left|r^{\prime}-\vec{r}\right|} e^{i\left(G_j-\vec{G}_i\right) \cdot\left(r^{\prime}-\vec{r}\right)}\right]}_{\frac{2 \pi}{\left|G_j-G_i\right|}} \\ & =\frac{1}{\Omega^2} \int_{\Omega} d^2 \vec{r} e^{i\left[\left(G_q-\vec{G}_p\right)+\left(\vec{G}_j-\vec{G}_i\right)\right] \cdot \vec{r}} \cdot \frac{2 \pi}{\left|\vec{G}_j-\vec{G}_i\right|} \\ & =\frac{1}{\Omega} \cdot \delta\left(\vec{G}_j+\vec{G}_q-\vec{G}_i-\vec{G}_p\right) \cdot \frac{2 \pi}{\left|\vec{G}_j-\vec{G}_i\right|} \\ & =\frac{2 \pi}{\Omega\left|\vec{G}_j-\vec{G}_i\right|} \underbrace{\delta\left[\left(\vec{G}_j+\vec{G}_q\right)-\left(\vec{G}_i+\vec{G}_p\right)\right]}_{\text {momentum conservation }}
\end{align}
...
\end{align}
\begin{align}
{\Sigma}^{(2),R}_{pq}(\omega) &\overset{\text{UHF}}{=} \frac{1}{2}\sum_{iab} \frac{\left( \left<pi||ab\right>\left<ab||qi\right>\right)}{\omega - \epsilon_{a} - \epsilon_{b} + \epsilon_{i} + i \eta} + \frac{1}{2}\sum_{ija} \frac{\left( \left<pa||ij\right>\left<ij||qa\right>\right)}{\omega - \epsilon_{j} - \epsilon_{i} + \epsilon_{a} + i \eta} \\
\implies {\Sigma}^{(2),R}(p, \omega) &= \sum_{iab} \frac{\left( 2\left<pi|ab\right> - \left<pi|ba\right>\right)^2}{\omega - \epsilon_{a} - \epsilon_{b} + \epsilon_{i} + i \eta} + \sum_{ija} \frac{\left( 2\left<pa|ij\right> - \left<pa|ji\right>\right)^2}{\omega - \epsilon_{j} - \epsilon_{i} + \epsilon_{a} + i \eta} \label{init_sose}\\
% & =  \sum_{\mathbf{K}_i \in O; \,\mathbf{K}_a , \mathbf{K}_b \in V} \frac{\left[ 2\left(
% \frac{4\pi\delta_{\mathbf K_p - \mathbf K_a, \mathbf K_i - \mathbf K_b}}{\Omega|\mathbf K_p - \mathbf K_a|^2}
% \times(1-\delta_{\mathbf K_p,\mathbf K_a})\right) - \left( \frac{4\pi\delta_{\mathbf K_p - \mathbf K_b, \mathbf K_i - \mathbf K_a}}{\Omega|\mathbf K_p - \mathbf K_b|^2} \times(1-\delta_{\mathbf K_p,\mathbf K_b})\right) \right]^2 }{\omega - \epsilon_{\mathbf{K}_a} - \epsilon_{\mathbf{K}_b} + \epsilon_{\mathbf{K}_i} + i \eta}\nonumber \\
%  &  + \sum_{\mathbf{K}_i,\mathbf{K}_j \in O;\, \mathbf{K}_a \in V} \frac{\left[ 2\left(\frac{4\pi\delta_{\mathbf K_p - \mathbf K_i, \mathbf K_j - \mathbf K_a}}{\Omega|\mathbf K_p-\mathbf K_i|^2} \times(1-\delta_{\mathbf K_p,\mathbf K_i})\right) - \left(\frac{4\pi\delta_{\mathbf K_p - \mathbf K_j, \mathbf K_i - \mathbf K_a}}{\Omega|\mathbf K_p - \mathbf K_j|^2} \times(1-\delta_{\mathbf K_p,\mathbf K_j})\right) \right]^2 }{\omega - \epsilon_{\mathbf{K}_i} - \epsilon_{\mathbf{K}_j} + \epsilon_{\mathbf{K}_a} + i \eta} \\
&= \left(\frac{4\pi}{\Omega}\right)^2 \left( \sum_{\mathbf{K}_i \in O; \,\mathbf{K}_a , \mathbf{K}_b \in V} \frac{\left[ \frac{2\delta_{\mathbf{K}_p - \mathbf{K}_a , \mathbf{K}_b - \mathbf{K}_i}}{|\mathbf{K}_p - \mathbf{K}_a|^2}\times(1-\delta_{\mathbf K_p,\mathbf K_a})\right]^2 }{\omega - \epsilon_{\mathbf{K}_a} - \epsilon_{\mathbf{K}_b} + \epsilon_{\mathbf{K}_i} + i \eta}  + \frac{\left[ \frac{\delta_{\mathbf{K}_p - \mathbf{K}_b , \mathbf{K}_a - \mathbf{K}_i}}{|\mathbf{K}_p - \mathbf{K}_b|^2}\times(1-\delta_{\mathbf K_p,\mathbf K_b})\right]^2 }{\omega - \epsilon_{\mathbf{K}_a} - \epsilon_{\mathbf{K}_b} + \epsilon_{\mathbf{K}_i} + i \eta} \right. \\
& \left. - \frac{4\delta_{\mathbf{K}_p - \mathbf{K}_a , \mathbf{K}_b - \mathbf{K}_i}\delta_{\mathbf{K}_p - \mathbf{K}_b , \mathbf{K}_a - \mathbf{K}_i}}{|\mathbf{K}_p - \mathbf{K}_a|^2 |\mathbf{K}_p - \mathbf{K}_b|^2}\times(1-\delta_{\mathbf K_p,\mathbf K_a})(1-\delta_{\mathbf K_p,\mathbf K_b}) \times \frac{1}{\omega - \epsilon_{\mathbf{K}_a} - \epsilon_{\mathbf{K}_b} + \epsilon_{\mathbf{K}_i} + i \eta} \right. \nonumber \\
& \left. + \sum_{\mathbf{K}_i,\mathbf{K}_j \in O;\, \mathbf{K}_a \in V} \frac{\left[ \frac{2\delta_{\mathbf{K}_p - \mathbf{K}_i , \mathbf{K}_j - \mathbf{K}_a}}{|\mathbf{K}_p-\mathbf{K}_i|^2}\times(1-\delta_{\mathbf K_p,\mathbf K_i})\right]^2 }{\omega - \epsilon_{\mathbf{K}_i} - \epsilon_{\mathbf{K}_j} + \epsilon_{\mathbf{K}_a} + i \eta} + \frac{\left[ \frac{\delta_{\mathbf{K}_p - \mathbf{K}_j , \mathbf{K}_i - \mathbf{K}_a}}{|\mathbf{K}_p - \mathbf{K}_j|^2}\times(1-\delta_{\mathbf K_p,\mathbf K_j})\right]^2 }{\omega - \epsilon_{\mathbf{K}_i} - \epsilon_{\mathbf{K}_j} + \epsilon_{\mathbf{K}_a} + i \eta}  \right. \nonumber\\
& \left. - \frac{4\delta_{\mathbf{K}_p - \mathbf{K}_i , \mathbf{K}_j - \mathbf{K}_a}\delta_{\mathbf{K}_p - \mathbf{K}_j , \mathbf{K}_i - \mathbf{K}_a}}{|\mathbf{K}_p-\mathbf{K}_i|^2 |\mathbf{K}_p - \mathbf{K}_j|^2}\times(1-\delta_{\mathbf K_p,\mathbf K_i})(1-\delta_{\mathbf K_p,\mathbf K_j}) \times \frac{1}{\omega - \epsilon_{\mathbf{K}_i} - \epsilon_{\mathbf{K}_j} + \epsilon_{\mathbf{K}_a} + i \eta} \right) \nonumber
\label{eq:self_energy_2nd_order}
\end{align}
We can also work in the sine-cosine basis, where we can directly implement \ref{init_sose} in a standard quantum chemistry approach, because now the ERIs have 8-fold symmetry. But we must take into account the correct mapping between the plane wave and sine-cosine basis, leading to
\begin{equation}
...
\end{equation}
\subsection{HF spectral functions}
We can define $\xi=-2 \times 2.837297 \times\left(\frac{3}{4 \pi}\right)^{1 / 3} N^{-1 / 3} r_{\mathrm{s}}^{-1}$ Hartrees.
\subsubsection{Blue solid curve}
\label{sec:blue_curve}
The blue curve is obtained by using a retarded greens function of the form
\begin{equation}
    G^R(p, \omega) = \frac{1}{\omega - \epsilon_p + i \eta}
\end{equation}
with $\epsilon_p$ the HF orbital energies that I have obtained with an SCF procedure, which never takes into account the Madelung correction. Then, I am plotting the spectral function $A(p, \omega) = -\frac{1}{\pi} \operatorname{Im} \left[ G^R(p, \omega + \frac{\xi}{2}) \right]$. 
\subsubsection{Orange dotted curve}
This one is obtained again by using a retarded greens function of the form
\begin{equation}
    G^R(p, \omega) = \frac{1}{\omega + \varepsilon_p + i \eta}
\end{equation}
where now the $\varepsilon_p$ are the HF orbital energies that you sent yesterday. Remember that you negated them, which is why the sign is reversed above. Then, I am plotting the spectral function $A(p, \omega) = -\frac{1}{\pi} \operatorname{Im} \left[ G^R(p, \omega + \frac{\xi}{4}) \right]$. Note that this is a perfect match for the one above for these two cases of the GPT and HOMO.
\subsubsection{Green dashed curve}
For this one, I took the exact same approach as in section \ref{sec:blue_curve}, with the only difference being that I defined
\begin{equation}
    G^R(p, \omega) = \frac{1}{\omega - (\epsilon_p-\varepsilon_F) + i \eta}
\end{equation}
where $\varepsilon_F$ is the Fermi energy (or the energy of my HOMO in HF) from my code, which is due to iPie.
\subsubsection{Note about the virtuals}
I gave 1.5 hour of effort to figuring out the connection between your virtual poles and mine, but I was not able to get them to match yet. Is it important that I get them to match now? I know it would be good, but all of the references just report the spectral functions for occupied orbitals, so I am not sure if this should be a priority.
