...
ndocc = wfn.doccpi()[0]
nmo = wfn.nmo()
print(f'Number of MOs: {nmo}, Number of occupied orbitals: {ndocc}')
SCF_E = wfn.energy()
eps = np.asarray(wfn.epsilon_a())
print('RHF Energy: %16.10f Ha' % SCF_E)
print('RHF MO Energies (a.u.):\n', eps)
...
for orbital in range(start_orbs + 1, start_orbs + num_orbs + 1):
    ...
    print('\nStarting EP2 for orbital HOMO - %d (orbital index %d)' % (ndocc - orbital - 1, orbital))
    ...
    print(f'Sigma 2p1h = {sigma1:12.8f}')

    tmp2 = (2 * MO[orbital, v, o, o] - MO[v, orbital, o, o])
    sigma2 = np.einsum('abr,rab,rab->', MO[o, o, orbital, v], tmp2, epsilon2)
    print(f'Sigma 2h1p = {sigma2:12.8f}')
    print(f'Total = {sigma1 + sigma2:12.8f}')

    # Build derivatives
    sigma_deriv1 = np.einsum('rsa,ars,ars->', MO[v, v, orbital, o], tmp1, epsilon1**2)
    sigma_deriv2 = np.einsum('abr,rab,rab->', MO[o, o, orbital, v], tmp2, epsilon2**2)
    deriv = -1 * (sigma_deriv1 + sigma_deriv2)

    # Z factor 
    Z = 1 / (1 - deriv)
    print(f'Z factor = {Z:12.8f}')
    ep2_arr.append(eps[orbital] + Z*(sigma1 + sigma2))
    ...
    #     tmp2 = (2 * MO[orbital, v, o, o] - MO[v, orbital, o, o])
    #     sigma2 = np.einsum('abr,rab,rab->', MO[o, o, orbital, v], tmp2, epsilon2)
    #     print(f'Iter {ep_iter+1:2d}: Sigma1 = {sigma1:12.8f}  Sigma2 = {sigma2:12.8f} Total = {sigma1 + sigma2:12.8f}')
