...
\end{figure}
\newpage
\subsection{Spectral function of the uniform electron gas (UEG)} The UEG is the paradigmatic metallic system \cite{Mahan2014-ad}, serving as a testbed for many-body methods hoping to describe metals. Its properties in the thermodynamic limit (TDL) are not known. Although the TDL value is of interest to the quantum chemist, in practice, solid-state calculations are performed using a finite k-point sampling. Therefore, the discussion below assumes finite supercells of the UEG. Green's function methods are well suited for computing the spectral function $A(\omega)$, since the spectral function
...
Because obtaining the exact self-energy is intractable, MBPT methods approximate $\Sigma$ through a perturbative expansion in the interaction strength. The GW approximation corresponds to making a perturbative expansion in the screened Coulomb interaction $W$ and truncating at first order. However, solving the Dyson equation perturbatively with truncation at first order fails to capture satellite features in core-level spectral functions of the UEG \cite{Holm1997-by, Caruso2016-us,McClain2016-md}.
...
However, the accuracy of cumulant expansion depends on the choice of $\Sigma$ in equation~\ref{eq:cumulant}. 
...
\subsubsection{PT2+C}
Second-order perturbation theory (PT2) admits the second-order self-energy, whose construction costs $O(N^5)$. Using $p, q\ldots$,  $i, j \ldots$, and $a, b \ldots$, to denote general, occupied, and virtual indices, respectively, the PT2 cumulant reads:
\begin{align}
...
BWs2 has been shown to accurately compute ground state properties of metals\cite{Chen2025-go}. Recently, Tew et al. put the BWs2 self-energy on firm theoretical grounds, by showing that the theory admits a Goldstone self energy, instead of the Dyson self-energy used by methods, such as GW\cite{Coveney2023-tr}. This implies that BWs2 obeys a Dyson equation like equation~\ref{eq:dyson}, but where $G_0$ is no longer due to a HF input, but rather to the "dressed" BWs2 one. This led me to derive the following form for the BWs2 cumulant as
\begin{equation}
    C_{pp}^{\mathrm{BWs2}}(t) = 
